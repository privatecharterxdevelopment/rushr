-- ============================================================================
-- SIGNALS SYSTEM SETUP - COMPLETE
-- ============================================================================
-- Run this file to set up the complete signals functionality
-- Includes: signal_sources table, signals table, and sample data
-- ============================================================================
...
-- ============================================================================
-- CONTRACTOR SIGNAL SUBSCRIPTIONS TABLE
-- ============================================================================
CREATE TABLE IF NOT EXISTS contractor_signal_subscriptions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Signal subscription indexes
CREATE INDEX IF NOT EXISTS idx_contractor_subscriptions_contractor ON contractor_signal_subscriptions(contractor_id);
CREATE INDEX IF NOT EXISTS idx_contractor_subscriptions_active ON contractor_signal_subscriptions(is_active) WHERE is_active = true;
...
-- Signal sources - read-only for authenticated users
...
-- Signals - read-only for authenticated users (contractors)
...
-- Contractor subscriptions - contractors can manage their own
DROP POLICY IF EXISTS "Contractors can view own subscriptions" ON contractor_signal_subscriptions;
CREATE POLICY "Contractors can view own subscriptions" ON contractor_signal_subscriptions
    FOR SELECT USING (auth.uid() = contractor_id);

DROP POLICY IF EXISTS "Contractors can insert own subscriptions" ON contractor_signal_subscriptions;
CREATE POLICY "Contractors can insert own subscriptions" ON contractor_signal_subscriptions
    FOR INSERT WITH CHECK (auth.uid() = contractor_id);

DROP POLICY IF EXISTS "Contractors can update own subscriptions" ON contractor_signal_subscriptions;
CREATE POLICY "Contractors can update own subscriptions" ON contractor_signal_subscriptions
    FOR UPDATE USING (auth.uid() = contractor_id);

DROP POLICY IF EXISTS "Contractors can delete own subscriptions" ON contractor_signal_subscriptions;
CREATE POLICY "Contractors can delete own subscriptions" ON contractor_signal_subscriptions
