-- =============================================================================
-- COMPLETE DIRECT CONTRACTOR OFFER SYSTEM + PAYMENT FIX
-- RUN THIS ENTIRE FILE IN SUPABASE SQL EDITOR
-- Safe to run multiple times (uses IF NOT EXISTS, DROP IF EXISTS, etc.)
-- =============================================================================

-- =============================================================================
-- PART 1: CREATE DIRECT_OFFERS TABLE
-- =============================================================================
...
-- =============================================================================
-- PART 2: CREATE DATABASE FUNCTIONS (WITHOUT CONVERSATION CREATION)
-- Conversations will be created AFTER payment succeeds
-- =============================================================================
...
BEGIN
  ...
  IF v_homeowner_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
...
-- Function to accept offer (NO conversation creation - happens after payment)
...
-- Function to accept counter-bid (NO conversation - happens after payment)
...
BEGIN
  ...
  SET homeowner_accepted_counter = true, status = 'agreement_reached',
      ...
      updated_at = NOW()
  WHERE id = p_offer_id AND contractor_response = 'counter_bid';
  IF NOT FOUND THEN RAISE EXCEPTION 'Offer not found or no counter-bid exists'; END IF;
END;
$$;
...
-- Notify contractor when they receive offer
...
-- =============================================================================
-- PART 4: UPDATE NOTIFICATIONS TABLE TYPE CONSTRAINT
-- =============================================================================
...
-- =============================================================================
-- PART 5: FIX accept_job_bid TO NOT CREATE CONVERSATION
-- Conversation will be created AFTER payment succeeds
-- =============================================================================
...
BEGIN
  ...
  IF v_job_id IS NULL THEN RAISE EXCEPTION 'Bid not found or access denied'; END IF;

  UPDATE job_bids SET status = 'accepted', updated_at = NOW() WHERE id = p_bid_id;
  UPDATE job_bids SET status = 'rejected', updated_at = NOW()
  WHERE job_id = v_job_id AND id != p_bid_id;

  UPDATE homeowner_jobs
  SET status = 'bid_accepted', accepted_bid_id = p_bid_id,
      contractor_id = v_contractor_id, estimated_cost = v_bid_amount, updated_at = NOW()
  WHERE id = v_job_id;

  -- NOTE: Conversation NOT created here - will be created by finalize_bid_after_payment() AFTER payment
END;
$$;

-- =============================================================================
-- PART 6: CREATE FINALIZE FUNCTIONS (CALL THESE AFTER PAYMENT SUCCEEDS)
-- =============================================================================

-- Finalize bid AFTER payment (creates conversation)
CREATE OR REPLACE FUNCTION finalize_bid_after_payment(p_bid_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF v_bid.id IS NULL THEN RAISE EXCEPTION 'Bid not found or not accepted'; END IF;

  INSERT INTO conversations (homeowner_id, pro_id, job_id, title, status, created_at, updated_at)
  VALUES (v_bid.homeowner_id, v_bid.contractor_id, v_bid.job_id::TEXT, v_bid.job_title, 'active', NOW(), NOW())
  ...
  UPDATE homeowner_jobs SET status = 'in_progress', updated_at = NOW() WHERE id = v_bid.job_id;
  RETURN v_conversation_id;
END;
$$;

-- Finalize offer AFTER payment (creates conversation)
CREATE OR REPLACE FUNCTION finalize_offer_after_payment(p_offer_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_conversation_id UUID;
BEGIN
  SELECT * INTO v_offer FROM direct_offers
  WHERE id = p_offer_id AND status IN ('accepted', 'agreement_reached');

  IF v_offer.id IS NULL THEN RAISE EXCEPTION 'Offer not found or not accepted'; END IF;
...
-- =============================================================================
-- PART 7: EXTEND PAYMENT_HOLDS TABLE TO SUPPORT OFFERS
-- =============================================================================

-- Add offer_id column
ALTER TABLE payment_holds ADD COLUMN IF NOT EXISTS offer_id UUID REFERENCES direct_offers(id) ON DELETE CASCADE;

-- Make job_id and bid_id optional
ALTER TABLE payment_holds ALTER COLUMN job_id DROP NOT NULL;
ALTER TABLE payment_holds ALTER COLUMN bid_id DROP NOT NULL;

-- Add constraint: must have either bid_id OR offer_id (not both)
ALTER TABLE payment_holds DROP CONSTRAINT IF EXISTS payment_holds_bid_or_offer_check;
ALTER TABLE payment_holds ADD CONSTRAINT payment_holds_bid_or_offer_check CHECK (
  (bid_id IS NOT NULL AND offer_id IS NULL) OR
  (bid_id IS NULL AND offer_id IS NOT NULL)
);

-- Update unique constraints
ALTER TABLE payment_holds DROP CONSTRAINT IF EXISTS payment_holds_bid_id_key;
ALTER TABLE payment_holds ADD CONSTRAINT payment_holds_offer_id_unique UNIQUE (offer_id);
ALTER TABLE payment_holds ADD CONSTRAINT payment_holds_bid_id_unique UNIQUE (bid_id);
...
-- =============================================================================
-- PART 8: ENABLE REALTIME FOR MESSAGING
-- =============================================================================

-- Enable realtime on conversations and messages tables
DO $$
BEGIN
  -- Check if conversations table is already in realtime publication
  IF NOT EXISTS (
...
SELECT '✅ COMPLETE! Direct offers system is now live with payment-before-conversation flow!' as status;
SELECT '📱 Messaging: Real-time chat enabled (conversations, messages, notifications)' as messaging_status;
SELECT '💳 Payment: Stripe escrow for both bids AND direct offers' as payment_status;
SELECT '🔔 Notifications: Contractors get bell notifications for new offers' as notification_status;
