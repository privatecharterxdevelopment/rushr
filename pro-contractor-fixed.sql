-- ============================================================================
-- RUSHR PRO CONTRACTOR SYSTEM - CLEAN INSTALLATION (FIXED)
-- ============================================================================
-- This creates the complete Pro contractor system independently
-- Run this AFTER basic auth is set up, but handles its own user_profiles
-- ============================================================================
...
-- ============================================================================
-- ENUMS (create directly, not in DO blocks)
-- ============================================================================
...
DROP TRIGGER IF EXISTS update_contractor_kyc_data_updated_at ON contractor_kyc_data;
CREATE TRIGGER update_contractor_kyc_data_updated_at
    BEFORE UPDATE ON contractor_kyc_data
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_contractor_documents_updated_at ON contractor_documents;
CREATE TRIGGER update_contractor_documents_updated_at
    BEFORE UPDATE ON contractor_documents
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_contractor_portfolio_updated_at ON contractor_portfolio;
...
DROP POLICY IF EXISTS "Contractors can manage their own profile" ON contractor_profiles;
...
DROP POLICY IF EXISTS "Contractors can manage their own KYC data" ON contractor_kyc_data;
CREATE POLICY "Contractors can manage their own KYC data" ON contractor_kyc_data
    FOR ALL USING (auth.uid() = id);

-- Document policies
DROP POLICY IF EXISTS "Contractors can manage their own documents" ON contractor_documents;
CREATE POLICY "Contractors can manage their own documents" ON contractor_documents
    FOR ALL USING (contractor_id = auth.uid());

-- Portfolio policies
DROP POLICY IF EXISTS "Contractors can manage their own portfolio" ON contractor_portfolio;
CREATE POLICY "Contractors can manage their own portfolio" ON contractor_portfolio
    FOR ALL USING (contractor_id = auth.uid());

DROP POLICY IF EXISTS "Public can view public portfolio items" ON contractor_portfolio;
...
BEGIN
    ...
    RAISE NOTICE 'Handles conflicts gracefully with existing homeowner system.';
END $$;