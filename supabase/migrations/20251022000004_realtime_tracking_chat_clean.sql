...
-- Drop tables first (CASCADE will remove policies, triggers, indexes automatically)
DROP TABLE IF EXISTS job_chat_messages CASCADE;
DROP TABLE IF EXISTS contractor_locations CASCADE;
...
CREATE INDEX idx_contractor_locations_job_id ON contractor_locations(job_id);
CREATE INDEX idx_contractor_locations_last_updated ON contractor_locations(last_updated_at DESC);
...
CREATE INDEX idx_job_chat_messages_job_id ON job_chat_messages(job_id, created_at DESC);
CREATE INDEX idx_job_chat_messages_sender ON job_chat_messages(sender_id);
CREATE INDEX idx_job_chat_messages_unread ON job_chat_messages(job_id) WHERE read_at IS NULL;
...
-- 5. RLS Policies for contractor_locations
...
-- 6. RLS Policies for job_chat_messages
...
-- 7. Function to update location timestamp automatically
CREATE FUNCTION update_contractor_location_timestamp()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION get_unread_message_count(p_job_id UUID, p_user_id UUID)
RETURNS INTEGER AS $$
DECLARE
...
-- 9. Add tracking_enabled field to homeowner_jobs if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
